...
def load_files(input_files: list, show: bool=False, debug: bool=False):
    if debug:
    ...
    ax = plt.gca()
    colormap = cm.viridis
    colorlist = [colors.rgb2hex(colormap(i)) for i in np.linspace(0, 0.9, len(input_files))] # len(input_files))]
    ...
    for i, f in enumerate(input_files):
        ...
        try:
            if f.endswith(".txt"):
                ...
                if show:
                    if 'Icoil1' in keys and 'Flow1' in keys:
                        lname = f.replace("_","-")
                        lname = lname.replace(".txt","")
                        lname = lname.split('/')
                        legends.append(f'{lname[-1]}')
                        # print(f'rename Flow1 to {lname[-1]}')
                        _df.plot.scatter(x='Icoil1', y='Flow1', grid=True, label=f'{lname[-1]}', color=colorlist[i], ax=ax)
                        # print(f'tttut')
            else:
